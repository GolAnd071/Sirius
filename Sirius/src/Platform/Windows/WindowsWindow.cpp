...
namespace Sirius {
	...
	static void GLFWErrorsCallback(int error, const char* description)
	{
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		SRS_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (s_GLFWWindowCount == 0)
		{
			SRS_PROFILE_SCOPE("glfwInit");
			int success = glfwInit();
			SRS_CORE_ASSERT(success, "Could not initialize GLFW!");
			glfwSetErrorCallback(GLFWErrorsCallback);
		}

		{
			SRS_PROFILE_SCOPE("glfwCreateWindow");
